...
# Uso: bash scripts/apply-reauth-implementation.sh
...
if [ -f "pronto-libs/src/pronto_shared/migrations/010_add_system_handoff_and_audit.sql" ]; then
    ...
    if command -v psql &> /dev/null && [ ! -z "${SUPABASE_DB_HOST:-}" ]; then
        ...
        if [ "$table_exists" = "t" ]; then
            ...
            echo "  bash bin/apply-migration.sh pronto-libs/src/pronto_shared/migrations/010_add_system_handoff_and_audit.sql"
        fi
    else
