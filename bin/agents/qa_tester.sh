...
if grep -rE "\.only\(|fit\(|fdescribe\(" tests/ e2e-tests/ src/clients_app/static/js/src/__tests__ src/employees_app/static/js/src/__tests__ --include="*.ts" --include="*.js" --include="*.py" --exclude-dir=node_modules > /dev/null 2>&1; then
