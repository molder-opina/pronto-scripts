#!/usr/bin/env python3
"""Transicionar estado de orden usando transiciones canónicas.

Estados canónicos:
  new -> queued -> preparing -> ready -> delivered -> paid
  y cancelación hacia cancelled según policy.

Uso:
  python pasar_estado.py --id <order_uuid|order_number> --status preparing
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cambiar estado de orden (canónico)")
    parser.add_argument("--id", required=True, help="UUID u order_number de la orden")
    parser.add_argument("--status", required=True, choices=CANON_STATUSES, help="Nuevo estado")
    parser.add_argument(
        "--actor-scope",
        default="system",
        choices=["waiter", "chef", "cashier", "admin", "system", "client"],
        help="Scope ejecutando la transición",
    )
    ...
    parser.add_argument(
        ...
        help="Método de pago (si --status paid)",
    )
    parser.add_argument("--payment-reference", help="Referencia de pago (opcional)")
    parser.add_argument(
        "--amount-received",
        type=float,
        help="Monto recibido en efectivo (si aplica)",
    )
    ...
    try:
        ...
        _print_error(args.json, f"Estado inválido: {args.status}")
        sys.exit(1)
    ...
    if target_status == OrderStatus.PAID:
        payload["payment_method"] = args.payment_method
        if args.payment_reference:
            payload["payment_reference"] = args.payment_reference
        if args.amount_received is not None:
            payload["amount_received"] = args.amount_received
    ...
    if status_int >= HTTPStatus.BAD_REQUEST:
        message = (
            ...
            or f"Error de transición (status={status_int})"
        )
